...
package
{
	...
	public class TiltingPaneN extends UIComponent
	{
		//---------------------------------------------------------------------------------------
		// constructor
		//---------------------------------------------------------------------------------------

		public function TiltingPaneN()
		{
			super();
		}
		...
		override protected function updateDisplayList(unscaledWidth:Number, unscaledHeight:Number):void
		{
			if(_content)
			{
				...
				perspectiveDistort(_content,_mask.graphics,_angle,centerX,0,contentWidth,contentHeight);					
				...
				if(!isNaN(borderColor) && !isNaN(borderThickness))
				{
					...
					drawPerspectiveFrame(g,_angle,centerX,0, contentWidth, contentHeight);
				}					
				...
				positionReflectionBitmap(_reflectionBitmap,_angle,centerX,0,contentWidth,contentHeight);
			}
		}
		
		
		
		private function perspectiveDistort( target:UIComponent, mask:Graphics, angle:Number, centerX:Number, yPosition:Number, frameWidth:Number, frameHeight:Number ):void
		{
			...
			m.ty = yPosition - verticalSheerEffect;
			...
			if(mask != null)
			{
				...
				drawPerspectiveFrame(mask,angle,centerX,yPosition,frameWidth, frameHeight);
				mask.endFill();
			}
			
		}
		
		private function drawPerspectiveFrame( g:Graphics, angle:Number, centerX:Number, yPosition:Number, frameWidth:Number, frameHeight:Number ):void
		{
			var p:Number = (Math.abs(angle)/90);
			p = Math.sqrt(p);
			...
			if(angle >= 0)
			{
				...
				g.moveTo(frameLeft,verticalShearEffect);
				g.lineTo(frameLeft,frameHeight + verticalShearEffect);
				g.lineTo(frameLeft + frameWidth*horizontalScale,frameHeight - verticalShearEffect);
				g.lineTo(frameLeft + frameWidth*horizontalScale,3*verticalShearEffect);
			...
			else
			{
				...
				g.moveTo(frameLeft,3*verticalShearEffect);
				g.lineTo(frameLeft, frameHeight - verticalShearEffect);
				g.lineTo(frameLeft+frameWidth*horizontalScale, frameHeight + verticalShearEffect);
				g.lineTo(frameLeft+frameWidth*horizontalScale, verticalShearEffect);
				g.lineTo(frameLeft,+3*verticalShearEffect);
			}
		}
		...
		private function positionReflectionBitmap(target:Bitmap,angle:Number,centerX:Number, yPosition:Number, frameWidth:Number, frameHeight:Number ):void
		{
			var m:Matrix = target.transform.matrix;
			...
			m.d = -1;
			m.tx = centerX - frameWidth/2 * horizontalScale;
			m.ty = yPosition - verticalSheerEffect + 2*frameHeight;
