...
//----------------------------------------------------------------------------------------------
		...
		private function mouseDownHandler(e:MouseEvent):void
		{
			...
			_mouseDownOffset = offsetForce.value - mouseX;
			prepareOffset(); 
		...
		{
			_dragForce.anchor = mouseX + _mouseDownOffset;
			e.updateAfterEvent();
		}
...
//----------------------------------------------------------------------------------------------
		...
		public function scrollTo(position:Number,offset:Number = 0):void
		{
			...
			prepareOffset();
			state = "searching";
			offsetForce.replaceForces(offsetForce.accelerateTo(findDirection(position,_currentScrollPosition)*offsetForce.vTerminal));			
		}
...
//----------------------------------------------------------------------------------------------
		...
		private function calculateLeftOffset():void
		{
			...
			{
				_leftAnchor.anchor = offsetForce.value - _positionOfLeftmostTile;
		...
		{			
			...
			if(bluriness == 0 || Math.round(offsetForce.velocity) == 0)
			{
				_itemLayer.filters = [];
			}
			else
			{
				_blur.blurY = 0;
				_blur.blurX = bluriness * Math.min(Infinity,Math.abs(Math.round(offsetForce.velocity/20)));
