package
{
	...
	*	it requires
	*/
	public class AnimatedRandomWalk extends UIComponent implements IFocusManagerComponent,
		IHistoryManagerClient
	{
		...
		*/
		public function AnimatedRandomWalk()
		{
			super();
			_rendererDataMap = {};
			_dataRendererMap = {};
			...
			_animator.animationSpeed = .25;
			_animator.initializeFunction = initializeLabelTarget;
		}
		...
		*/
		...
		private function removeInstances(instances:Array):void
		{
			for(var i:int=0;i<instances.length;i++)
			 ...
			 _animator.releaseTarget(instances[i]);
		}
		...
		{
			var inst:UIComponent = UIComponent(target.item);
			inst.x = target.x + target.unscaledWidth/2;
			inst.y = target.y;
			inst.setActualSize(0,target.unscaledHeight);
			inst.alpha = 0;
			inst.validateNow();
		}
		...
		*/
 		...
 		private function generateLayout():void
 		{
			...
			{			
				if(_highlightedNode == null)
				{
					...
					_animator.releaseTarget(_highlight);
				...
				else
				{
					...
					var selectedTarget:LayoutTarget = _animator.targetFor(highlightedInstance);
					_highlight.x = selectedTarget.x;
					_highlight.y = selectedTarget.y;
					_highlight.setActualSize(selectedTarget.unscaledWidth,selectedTarget.unscaledHeight);
				}
			}
		}
		...
		private function renderStack(instances:Array, selectedIndex:Number, left:Number):StackDetails
		{
			...
			var selectedTarget:LayoutTarget = null;
		...
		private function alignStack(instances:Array, details:StackDetails,selectionBaselineTarget:Number):Number
		{
			...
			for(i = 0;i<instances.length;i++)
			{
				inst = instances[i];
				target = _animator.targetFor(inst);
				target.y = target.y + verticalOffset;
			}
