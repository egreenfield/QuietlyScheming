...
//----------------------------------------------------------------------------------------------
		...
		public function prepareOffset(value:Number = 0):void
		{
			offsetForce.replaceForces(); 
...
//			offsetForce.replaceForces(offsetForce.solveFor(offsetForce.value - (tilePosition-focusPoint),Clock.global.t));			
			var f:ArriveAtForce = new ArriveAtForce(offsetForce.value + (tilePosition-focusPoint))
			offsetForce.replaceForces(f);
		}
		...
		public function scrollTo(position:Number,offset:Number = 0):void
		{
			...
			_state = "searching";
			_interaction.abort();
			var d:Number = findDirection(currentScrollPosition,position)
			var f:PedalToMetalForce = new PedalToMetalForce(d);
...
//----------------------------------------------------------------------------------------------
		...
		public function update():void
		{
			...
			while(rcOffset.left > leftEdge)
			{
				...
				{
					completeSearch(rcOffset.left,tileWidth);
				}
			}
