...
*
*/
    public function closestPointToBezier( _curve:IParametric, _p:Point ):Number
    {
      if( _curve == null )
      {
      	return 0;
      ...
      var x0:Number     = _curve.getX(0);
      var y0:Number     = _curve.getY(0);
      var deltaX:Number = x0-_p.x;
      var deltaY:Number = y0-_p.y;
      var d0:Number     = Math.sqrt(deltaX*deltaX + deltaY*deltaY);
      
      var x1:Number = _curve.getX(1);
      var y1:Number = _curve.getY(1);
      deltaX        = x1-_p.x;
      deltaY        = y1-_p.y;
      var d1:Number = Math.sqrt(deltaX*deltaX + deltaY*deltaY);
      
      var n:uint = _curve.degree;  // degree of input Bezier curve
